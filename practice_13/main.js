...
const { group, text_map } = CREATE.createSphereMesh(
  data.sun.name,
  data.sun.R,
  data.sun.URL,
  data.sun.color
);
// console.log(CREATE.createSphereMesh(data.sun.name, data.sun.R, data.sun.URL));
// console.log(sun, sun_map);
const sun = group;
text_map.wrapS = THREE.RepeatWrapping;
text_map.wrapT = THREE.RepeatWrapping;
sunOrib.add(sun);
...
data.planet.forEach((planet) => {
  ...
  if (planet.name === "Earth") {
    ...
    const star = CREATE.createSphereMesh(
      planet.name,
      planet.R,
      planet.URL,
      planet.color
...
function render() {
  ...
  // 让太阳流动
  text_map.offset.x -= 0.001;
  text_map.offset.y += 0.001;

  // solar_system[0].children.forEach((planet) => {
  sunOrib.children.forEach((planet) => {
    planet.rotateY(0.02);
    if (planet.angle) {
      if (planet.name == "earth") {
        planet.rotateY(0.01);
